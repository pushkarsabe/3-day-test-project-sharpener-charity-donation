...
async function submitData() {
    ...
    if (name == "" || email == "" || phoneNumber == "" || password == "") {
        ...
        try {
            const response = await axios.post(`http://${HOST}:5000/admin/signup`, obj);

            console.log('data added');
            console.log('response data = ' + JSON.stringify(response));
            console.log('response name = ' + response.data.existingAdmin.name);
            console.log('response email = ' + response.data.existingAdmin.email);
            console.log('response phoneNumber = ' + response.data.existingAdmin.phoneNumber);
